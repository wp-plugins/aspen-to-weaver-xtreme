...
// download converted settings file - 2/2/15
	...
	if (! wp_verify_nonce($nonce, 'aspen2wx_download')) {
		@header('Content-Type: ' . get_option('html_type') . '; charset=' . get_option('blog_charset'));
	...
	if ( $ext != 'wxplus') {
		$aspen2wx_opts = get_option('aspen2wx_settings', array());
		//echo '<pre>'; print_r($aspen2wx_opts); echo ('</pre>');
		$weaverx_opts['weaverx_base'] = $aspen2wx_opts['wx_converted'];
